 ...
 * dom方式处理xml
 */
public class DomCaseTest {
    private static Logger logger = LoggerFactory.getLogger(DomCaseTest.class);
    ...
    public void parse() throws Exception {
        ...
        //解析xml文件为文档
        Document document = builder.parse(DomCaseTest.class.getResourceAsStream("../../../resources/xml/student.xml"));
...
//        NodeList list = document.getElementsByTagNameNS("http://www.w3.org/TR/html4/school/", "student");//第1个学生
        for (int i = 0; i < list.getLength(); i++) {
            ...
            for (int j = 0; j < node.getAttributes().getLength(); j++) {
                logger.info(node.getAttributes().item(j).getNodeName() + ":" + node.getAttributes().item(j).getTextContent());
            }
